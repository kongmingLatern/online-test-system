<template>
  ...
  >
    ...
    >
      ...
      <Select v-if="item.type === 'select'" :type="sort" @change="getBaseId" />
    ...
    <div v-if="sort !== '' && formState[sort].questionAnswerList">
      <p>选项：</p>
      <a-form-item
    ...
    <div v-if="sort !== '' && formState[sort].questionCorrectList">
      <p>答案：</p>
      <a-form-item
        v-for="answer in formState[sort].questionCorrectList"
        :key="answer.key"
      >
        <a-input
          v-model:value="answer.values"
    ...
    <div text-right>
      <a-button class="button" html-type="submit" rounded> 提交信息 </a-button>
    </div>
  </a-form>
</template>
...
let finishImport: (file: File) => void = inject('import') ?? (() => {});
let finishAddForm: (file: File) => void = inject('select') ?? (() => {});
const store = useBase();
...
const onFinish = (values: any) => {
  ...
  if (props.sort === 'question') {
    ...
    if (formState[props.sort] !== null) {
      formState[props.sort].questionCorrectList = getValueByObject(
        formState[props.sort],
        'questionCorrectList'
      );
      formState[props.sort].questionAnswerList = getValueByObject(
        formState[props.sort],
        'questionAnswerList'
      );
    }
