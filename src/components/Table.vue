<template>
  <a-table
    ...
    :scroll="{ x: '100%', y: '100%' }"
  >
    <template #bodyCell="{ column, _, record, index }">
      <template v-if="column.dataIndex === 'no'">
        <span>{{ index + 1 }}</span>
      </template>
      <template v-if="column.dataIndex === 'status'">
        <a-tag v-if="record.status === '考完了'" color="red">
          {{ record.status }}
        </a-tag>
        <a-tag v-if="record.status === '考试中'" color="green">
          {{ record.status }}
        </a-tag>
      ...
      <template v-if="column.dataIndex === 'questionAnswer'">
        <span>{{ record.questionAnswer }}</span>
      </template>
      <template v-if="column.dataIndex === 'questionCorrect'">
      ...
      <template v-if="column.dataIndex === 'delete'">
        <a-button type="danger" rounded @click="deleteItem(record)">
          删除
        </a-button>
      </template>
      <template v-if="column.dataIndex === 'deleteAndAuth'">
        <a-space>
          <a-button type="danger" rounded @click="authItem(record)">
            授权
          </a-button>
        </a-space>
      </template>
      ...
      <template v-if="column.dataIndex === 'grade'">
        ...
        <a-tag v-else-if="record.grade >= 60" color="green" text-sm>
          {{ record.grade }}
        </a-tag>
      </template>
      <template v-if="column.dataIndex === 'allocation'">
        <a-button type="danger" @click="allocationTask(record)">
          分配考试
        </a-button>
      </template>
    </template>
